...
void checkBrightness() {
  if (LCD_BRIGHT == 11) {                         // если установлен автоматический режим для экрана (с)НР
    if (readBrightness() < BRIGHT_THRESHOLD) {   // если темно
...
void modesTick() {
  ...
  if (button.isSingle()) {    // одинарное нажатие на кнопку
    ...
    if (mode >= 240) {
      ...
      do {
        mode++;
        if (mode > 10) mode = 0;
#if ((CO2_SENSOR_MHZ19 == 0 || CO2_SENSOR_MQ2)&& mode == 1)
        mode = 3;
#endif
  ...
  if (button.isDouble()) {                    // двойное нажатие (с)НР ----------------------------
    if (mode > 0 && mode < 11) {              // Меняет пределы графика на установленные/фактические максимумы (с)НР
      ...
      mode0scr++;
      if ((CO2_SENSOR_MHZ19 == 0 || CO2_SENSOR_MQ2) && mode0scr == 1) mode0scr++;
...
void readSensors() {
  ...
  dispTemp = dht.getTemperature();
  dispHum = dht.getHumidity();
#endif

#if SENSOR_BME280
  bme.takeForcedMeasurement();
  dispHum = bme.readTemperature();
  dispTemp = bme.readHumidity();
...
#if CO2_SENSOR_MQ2
  dispCO2 = analogRead(CO2_SENSOR_MQ2_PIN);  
#endif
}
...
#if (CO2_SENSOR_MHZ19 == 1 || CO2_SENSOR_MQ2)
