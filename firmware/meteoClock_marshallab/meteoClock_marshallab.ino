...
// #define STASSID "Xpert"
// #define STAPSK "Xpert54991198"
#endif
...
#define CO2_SENSOR_MHZ19  0       // включить или выключить поддержку/вывод с датчика СО2 (1 вкл, 0 выкл) MHZ19
#define CO2_SENSOR_MQ2    1       // включить или выключить поддержку/вывод с датчика СО2 (1 вкл, 0 выкл) MHZ19
...
// пины
#define BACKLIGHT D7//13

//#define PHOTO_R A0  // пин фоторезистора
#define PHOTO_N D3
#define PHOTO_P D4

#define BTN_PIN 15 //D8 //15  // пин кнопки притянутый до земли резистором 10 кОм (сенсорный ТТР223 можно без резистора)
...
void setup() {
  ...
  if (!otaFlag) {
    if (mqtt_server.fromString(mqtt_ip)) {  //Указан корректный ip адрес
      ...
      client.set_server(mqtt_ip, mqtt_port.toInt());
  ...
  // dht.setup(17);
  // use this instead: 
  dht.setup(SENSOR_DHT_PIN, DHTesp::DHT22); // Connect DHT sensor to GPIO 17
#endif  

#if (CO2_SENSOR_MHZ19 == 1)
    lcd.setCursor(0, 0);
    lcd.print(F("MHZ-19... "));
    Serial.print(F("MHZ-19... "));
    mhz19.begin(MHZ19_TX, MHZ19_RX);
...
void loop() {
  byte btntemp = (digitalRead(BTN_PIN));
  ...
  if (WiFi.status() == WL_CONNECTED && otaFlag_) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      if (testTimer(RTCsyncTimerD, RTCsyncTimer))
        if (timeStatus() == timeSet) rtc.adjust(DateTime(year(), month(), day(), hour(), minute(), second()));
